...
module.exports = function (app) {
  ...
  app.get('/apiv1/notification', sendNotification.bind(null, app));
};
...
function postShared(app, req, res, next) {
  ...
  app.config.save();
  if (repo.shared) {
    discovery.repoAvailable(app.config.server.description, 8001, repo);
  } else {
...
function sendNotification(app, req, res, next) {
  console.log("sendNotification", req.query);

  var reposWithName = app.config.repos.filter(function(repo) {
    return repo.name === req.query.name && repo.shared;
  });
  if (!reposWithName.length) {
